...
export function listingRejectedEmail({
  ...
  if (isProActivity) {
    return baseEmail({
      ...
      body: `
        ...
        <p style="margin:0 0 16px;">Votre annonce <strong style="color:#1a1b25;">« ${escapeHtml(listingTitle)} »</strong> semble correspondre à une activité professionnelle (prestations, services réguliers, vocabulaire pro, volume de publications, etc.).</p>
        ...
        ${reason ? `<div style="background:#f5f2ff;border-left:3px solid #2f6fb8;border-radius:0 8px 8px 0;padding:16px 20px;margin:0 0 16px;text-align:left;"><p style="font-size:14px;color:#424751;line-height:1.7;margin:0;">${escapeHtml(reason)}</p></div>` : ""}
  ...
  return baseEmail({
    ...
    body: `
      <p style="margin:0 0 16px;">Bonjour <strong style="color:#1a1b25;">${escapeHtml(name)}</strong>,</p>
      <p style="margin:0 0 16px;">Nous n'avons pas pu publier votre annonce <strong style="color:#1a1b25;">« ${escapeHtml(listingTitle)} »</strong> car elle ne respecte pas nos conditions de publication.</p>
      ${reason ? `<div style="background:#f5f2ff;border-left:3px solid #2f6fb8;border-radius:0 8px 8px 0;padding:16px 20px;margin:0 0 16px;text-align:left;"><p style="font-size:14px;color:#424751;line-height:1.7;margin:0;">Motif : ${escapeHtml(reason)}</p></div>` : ""}
