  ...
  return baseEmail({
    ...
    body: `
      ...
      <div style="background:#eef4fb;border-left:3px solid #2f6fb8;border-radius:0 8px 8px 0;padding:16px 20px;margin:0 0 16px;text-align:left;">
        <p style="font-size:15px;color:#1a1b25;font-weight:700;margin:0 0 8px;">${escapeHtml(firstName)} ${escapeHtml(lastName)}</p>
      ...
      ${
        message
          ? `<div style="background:#f8fafc;border-radius:8px;padding:14px 18px;margin:0 0 16px;text-align:left;">
               <p style="font-size:13px;color:#424751;margin:0;font-style:italic;">« ${escapeHtml(message)} »</p>
             </div>`
          : ""
      }
      <p style="margin:0;">Rappelez-le au <strong>${escapeHtml(phone)}</strong>, puis passez la demande en « Contacté » dans l'administration.</p>
    `,
