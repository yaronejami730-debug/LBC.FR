...
export default function EmailScreen() {
  ...
  return (
    <View className="flex-1 bg-app p-4">
      ...
      <View className="bg-surface-container rounded-xl px-3 py-3 mb-3 flex-row items-center justify-between">
        <Text className="text-on-surface">{user?.email}</Text>
        {user?.emailVerified ? (
          <View className="flex-row items-center">
            <Ionicons name="checkmark-circle" size={18} color={colors.success} />
            <Text className="text-success text-xs font-semibold ml-1">Vérifiée</Text>
          </View>
        ) : (
          <Text className="text-primary text-xs font-semibold">Non vérifiée</Text>
        )}
      </View>
