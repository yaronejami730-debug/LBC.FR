...
export default function MesAnnoncesScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-app" edges={["top"]}>
      ...
      {/* Segmented control du design system */}
      <View className="px-4 pb-3">
        <SegmentedControl
          segments={TABS.map((t) => ({ value: t.key, label: t.label, count: counts[t.key] ?? undefined }))}
          value={tab}
          onChange={(next) => { setTab(next); setLoading(true); }}
        />
      </View>
...
function Card({ item, onPress }: { item: Listing; onPress: () => void }) {
  ...
  return (
    <View className="border border-line rounded-card bg-surface overflow-hidden">
      <Pressable onPress={onPress} className="flex-row p-3 active:opacity-80">
        ...
        <View className="flex-1 ml-3">
          ...
          <View className="flex-row items-center mt-1">
            <Text className="text-primary text-xl font-extrabold mr-2">{formatPrice(item.price)}</Text>
      ...
      {/* Stats footer */}
      <View className="flex-row border-t border-line px-3 py-2.5">
...
function Badge({ color, label }: { color: "amber" | "red" | "gray"; label: string }) {
  const bg = color === "amber" ? "bg-primary-light" : color === "red" ? "bg-danger/10" : "bg-surface-container";
  const fg = color === "amber" ? "text-primary" : color === "red" ? "text-danger" : "text-on-surface-variant";
  return (
